...
"""
#see shape
sound, a = librosa.load(os.path.join(data_dir, arr[9]))
print(sound.shape)
print(a)
features = extract_features(sound)

print(features.shape)
print(sound.shape[0]*30/a)
"""


count = 0
with h5py.File("audio_features_6class(1pad).hdf5","w") as hdf:
    for i in pbar:
        count+=1
        sound, _ = librosa.load(os.path.join(data_dir, i))
        features = extract_features(sound)
        if not (features.shape[0]<30 or features.shape[0]>160): #if audio is smaller than 1 sec discard it
            name = str(i.split('.')[0])
