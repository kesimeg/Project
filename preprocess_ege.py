...
def extract_features(sound_clip, bands = 60, frames = 41):
    ...
    log_specgrams = np.concatenate((log_specgrams, log_specgrams[-2:]), axis=0) #add last two frames again because of frame mistmatch
...
count = 0
with h5py.File("audio_features_4class_long.hdf5","w") as hdf:
    for i in pbar:
        count+=1
        if  i.find("ANG")!=-1 or i.find("HAP")!=-1  or i.find("SAD")!=-1 or i.find("NEU")!=-1:
