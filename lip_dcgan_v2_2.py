...
class Generator(nn.Module):
    ...
    def forward(self,ref_image,audio_data,noise):
        ...
        y, _ = self.noise_lstm(noise.view((batch_size,-1,100)), (h1, c1))
...
netG = Generator(128,256).to(device)
...
class Discriminator_img(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            # input is (nc) x 64 x 64
            nn.Conv2d(nc, self.ndf, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
...
"""
now=datetime.now()
plot_file="lip_gan_v2.2_"+now.strftime("%d_%m_%Y_%H:%M:%S")+".csv"



if not os.path.exists("./Img_folder/lip_ganv2_2"):
    os.mkdir("./Img_folder/lip_ganv2_2")

if not os.path.exists("./Models/lip_ganv2_2"):
    os.mkdir("./Models/lip_ganv2_2")
...
for epoch in range(opt.niter):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        ref_image,image_y,audio_data, _ = data
        ...
        noise = torch.randn(step_size,100, device=device)
        fake = netG(ref_image,audio_data,noise)
        ...
        """
        for p in netG.parameters(): #,model._all_weights[0]): #prints gradients below
            #if n[:6] == 'weight':
        ...
        if i<3:
            ...
            vutils.save_image(image_y[0,half:half+num_save,:,:,:],
                    '%s/train/real_samples_epoch_%03d_seq%d.png' % ("./Img_folder/lip_ganv2_2",epoch,i),
                    normalize=True)
            vutils.save_image(ref_image[0,:,:,:],
                    '%s/train/reference_samples_epoch_%03d_seq%d.png' % ("./Img_folder/lip_ganv2_2",epoch,i),
                    normalize=True)
            vutils.save_image(fake[0,half:half+num_save,:,:,:],
                    '%s/train/fake_samples_epoch_%03d_seq%d.png' % ("./Img_folder/lip_ganv2_2", epoch,i),
                    normalize=True)
        i+=1
    ...
    with torch.no_grad():
        ...
        for sample in test_loader:
            ref_image,image_y,audio_data,_ = sample
            step_size = image_y.size(1)
            t_vid=np.arange(0,step_size,5).shape[0]
            ...
            if i<3:
                ...
                vutils.save_image(image_y[half:half+num_save,:,:,:],
                        '%s/test/real_samples_epoch_%03d_seq%d.png' % ("./Img_folder/lip_ganv2_2",epoch,i),
                        normalize=True)
                vutils.save_image(ref_image[0,:,:,:],
                        '%s/test/reference_samples_epoch_%03d_seq%d.png' % ("./Img_folder/lip_ganv2_2",epoch,i),
                        normalize=True)
                vutils.save_image(fake[0,half:half+num_save,:,:,:],
                        '%s/test/fake_samples_epoch_%03d_seq%d.png' % ("./Img_folder/lip_ganv2_2", epoch,i),
                        normalize=True)
            i+=1
    ...
    print(data)
    if epoch%2==0:
        torch.save(netG.state_dict(), '%s/netG_epoch_%d.pth' % ("Models/lip_ganv2_2", epoch))
        torch.save(netD_img.state_dict(), '%s/netD_epoch_%d.pth' % ("Models/lip_ganv2_2", epoch))
